        ...
        for (let i = 0; i < sd.songs.length; i++) {
            ...
            while (1) {
                ...
                if (typeof reason === "string") {
                    ...
                    (i + 1) % 5 == 0 &&
                        tts(
                            `下载成功, 第 ${i + 1} 个, 共 ${sd.songs.length} 个`
                        );
                    log(
                        `下载成功: ${m.name} 第 ${i + 1} 个, 共 ${
                            sd.songs.length
                        } 个`
                    );
    ...
    ncmStatusCheck(
        ...
        .then(async resp => {
            ...
            if (
                ...
                songs.forEach(item => {
                    items[
                        `${item.name} 由 ${m.ar.map(ar => ar.name)} 演唱 id ${
                            item.id
                        }`
                    ] = item.id;
                });
